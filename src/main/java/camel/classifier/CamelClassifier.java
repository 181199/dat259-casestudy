...
public class CamelClassifier {
    ...
    public static void main(String[] args) throws Exception {
        ...
        filter.setDictionaryFileToSaveTo(new File("./dataset/features_camel.csv"));
        //filter.setStemmer(stemmer);
        filter.setStopwordsHandler(stopword);

        Instances keywords = Filter.useFilter(SBRs, filter);

        Map<String, Double> scored_words = scoreWords(SBRs, NSBRs, "./dataset/features_camel.csv");
        ...
        //sample.randomize(new Random(42));
        sample.stratify(numFolds);

        Instances trainingSet = sample.trainCV(numFolds, 0);
        Instances testSet= sample.testCV(numFolds, 0);

        RandomForestClassifier.classify(trainingSet, testSet, "camel");

        NaiveBayesClassifier.classify(trainingSet, testSet, "camel");

        SVMClassifier.classify(trainingSet, testSet, "camel");
    }
    ...
    public static Map<String, Double> scoreWords(Instances SBRs, Instances NSBRs, String keywordsFile){
        ...
        String[] keywords = new String[143];
        ...
        for(String w : keywords){
            ...
            // No support function
            score = Math.max(0.01, Math.min(0.99, (probabilitySBRs/(probabilitySBRs + probabilityNSBRs))));
    ...
    public static Instances loadDataset(String path) {
        ...
        try {
            ...
            Logger.getLogger(CamelClassifier.class.getName()).log(Level.SEVERE, null, ex);
        }
