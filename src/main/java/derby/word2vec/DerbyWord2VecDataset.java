 ...
 *
 */
public class DerbyWord2VecDataset {
	...
	public static void main(String[] args) throws Exception {
        ...
        Word2Vec vec = getWord2Vec("/Users/anja/Desktop/DAT259/GoogleNews-vectors-negative300.bin.gz");
        writeWord2VectorsGoogle("./dataset/derby_mod.csv", "./dataset/derby_wordvec.csv", vec);
	}
    ...
    public static Word2Vec getWord2Vec(String path) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        File googleModelFile = new File(path);
   ...
   private static String getWordVectorsAndLabel(INDArray vec, int index) {
    ...
    public static Collection<String> normalizeText(String text){
        ...
        input_text = input_text.replaceAll("[/]"," ");
        input_text = input_text.replaceAll(";"," ");
        //input_text = input_text.substring(0, Math.min(input_text.length(), 300));
        ...
        labels = labels.parallelStream().filter(label ->  !StopWords.getStopWords().contains(label.trim())).filter(label->label.length()<=300).collect(Collectors.toList());
        return labels;
    }
